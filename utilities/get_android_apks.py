...
hg_url = requests.get(
    f"https://hg.mozilla.org/releases/mozilla-release/rev/{final_version}"
).content
...
result_id_json = requests.get(
    f"https://treeherder.mozilla.org/api/project/mozilla-release/push/?full=true&count=10&revision={revision}",
    headers=headers,
).json()
result_id = result_id_json.get("results")[0].get("id")

fenix_jobs = requests.get(
    "https://treeherder.mozilla.org/api/project/mozilla-release/jobs/?job_group_symbol=fenix-debug&count=1000",
    headers=headers,
).json()
...
for job, id in download_ids.items():
    data = requests.get(
        f"https://treeherder.mozilla.org/api/project/mozilla-release/jobs/{id}/",
        headers=headers,
    ).json()
...
with open(
    path.resolve() / f"android-debug-test-v{args.firefox_version}.apk", "wb"
) as file:
