...
@pytest.fixture
def navigate_using_url_bar(selenium):
        ...
        with selenium.context(selenium.CONTEXT_CHROME):
            ...
            if use_clipboard:
                ActionChains(selenium).move_to_element(el).pause(1).click().pause(1).key_down(
                    cmd_ctrl
                ).send_keys("v").key_up(cmd_ctrl).send_keys(Keys.ENTER).perform()
            else:
                el.click()
                el.send_keys(text)
...
def copy_and_paste_string_to_url_bar(selenium, navigate_using_url_bar):
    cmd_ctrl = Keys.COMMAND if sys.platform == "darwin" else Keys.CONTROL
